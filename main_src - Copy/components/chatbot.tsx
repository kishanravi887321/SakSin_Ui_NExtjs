...
export function Chatbot() {
  ...
  const handleChatSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL1 || 'http://localhost:8005'}/chat/get/`, {
        method: 'POST',
        headers: {
  ...
  const handleFeedSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/users/chat/feed/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
                    ...
                    <div className="flex-1 flex flex-col">
                      ...
                      <div className="mb-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {/* Refeed Button - Always show for testing, but you can add apiKey condition later */}
                          {!isRefeedMode && (
                            <Button
